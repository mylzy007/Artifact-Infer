...
class Qwen3Attention(BaseService, nn.Module):
    @property
    def name(self):
...
class Qwen3Model(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [Qwen3DecoderLayer(layer_id, config) for layer_id in range(config.num_hidden_layers)]
        )
...
class Qwen3ForCausalLM(BaseService, nn.Module):
