...
class Qwen3Model(nn.Module):
    ...
    def __init__(
        ...
        self.layer_start = pp_rank * layers_per_stage + min(pp_rank, remainder)
        self.layer_end = self.layer_start + layers_per_stage + (1 if pp_rank < remainder else 0)
        ...
        self.layers = nn.ModuleList(
            [Qwen3DecoderLayer(config) for _ in range(self.layer_start, self.layer_end)]
        )
...
class Qwen3ForCausalLM(Artifact, nn.Module):
    ...
    def __init__(self, config: Qwen3Config, pp_rank: int = 0, pp_size: int = 1) -> None:
        super().__init__()
        self.pp_rank = pp_rank
        self.pp_size = pp_size
        ...
        if self.model.is_last_rank:
            ...
            if config.tie_word_embeddings and self.model.is_first_rank:
    ...
    def forward(
        ...
        return self.model(input_ids, positions, hidden_states, residual)
