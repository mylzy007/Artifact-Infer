...
from workshop.nanovllm_moe_orch.services.engine.sequence import Sequence
...
class Qwen3Attention(Artifact, nn.Module):
    ...
    def __init__(
        ...
        from workshop.nanovllm_moe_orch.services.utils.parallel import get_tp_world_size
...
def _resolve_rope_theta(config) -> float:
    """HF Qwen3 may store rope_theta nested in rope_scaling = {'rope_theta': ..., 'rope_type': 'default'}.
    `get_rope` is `@lru_cache`d → can't accept a dict for rope_scaling, so we extract theta and pass None."""
    theta = getattr(config, "rope_theta", None)
    if theta is None:
        rs = getattr(config, "rope_scaling", None) or {}
        theta = rs.get("rope_theta", 1000000)
    return float(theta)
